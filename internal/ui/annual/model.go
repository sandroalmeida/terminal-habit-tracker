...
func (m Model) LoadAnnualLogs() tea.Msg {
	startOfYear := time.Date(m.CurrentYear, 1, 1, 0, 0, 0, 0, time.Local)
	nextYear := startOfYear.AddDate(1, 0, 0)
	logs, err := m.LogRepo.GetLogsBetween(startOfYear, nextYear)
	if err != nil {
		return err
	}
	return logs
...
func (m Model) Update(msg tea.Msg) (Model, tea.Cmd) {
	...
	case map[int]map[string]bool:
		...
		for _, habit := range m.Habits {
			...
			s := m.StatsService.CalculateAnnualStats(habitLogs, m.CurrentYear, habit.GoalTarget)
			stats[habit.ID] = s
		}
		m.Stats = stats
...
func (m Model) View() string {
	...
	headerStr := fmt.Sprintf("%d", m.CurrentYear)
	s += lipgloss.NewStyle().Bold(true).Foreground(lipgloss.Color("205")).Render(headerStr) + "\n\n"
	...
	for i, habit := range m.Habits {
		...
		if habit.GoalTarget > 0 {
			annualGoal = 52 * habit.GoalTarget
		}
		goalStr := "∞"
		if annualGoal > 0 {
			goalStr = fmt.Sprintf("%d", annualGoal)
		}
